...
const TextField = ({
  ...
  return (
    <div className="w-full text-sm relative space-y-1">
      ...
      {/* Text Field */}
      <div
        className={`${errors.length ? 'ring-1 ring-red-500' : isFocused ? 'ring-1 ring-blue-600' : ''} ${
        ...
        {type !== 'textarea' ? (
          <div className="w-full flex justify-between items-center">
            ...
            {type === 'password' && (
              ...
              >
                {showPassword ? <EyeSlashIcon className="w-4 h-4" /> : <EyeIcon className="h-4 w-4" />}
              </button>
            )}
          </div>
        ) : (
